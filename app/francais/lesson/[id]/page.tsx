...
import { getLessonById, getRandomQuestions, type Question } from '@/src/data/francaisLessons';
...
export default function FrancaisLessonPage() {
  const params = useParams();
  const router = useRouter();
  const lessonId = parseInt(params.id as string);
  
  const { updateLessonProgress, addXP, updateStreak, totalXP, currentStreak, bestStreak } = useLessonProgress('francais');
  ...
  const handleSessionComplete = (finalScore: number, totalQuestions: number, correctAnswers: number) => {
    ...
    router.push(`/francais/lesson/${lessonId}/complete?score=${finalScore}&total=${totalQuestions}&correct=${correctAnswers}`);
  };
